...
1. Detects which product package changed
2. Runs `bricks updateci` to detect version bumps and cascade impacts
3. Scans `environments/**` to list which deployments reference the changed package
4. Posts a markdown table as a PR comment

**Honest note on input diffs:** The cascade impact table is generated by `bricks updateci` output plus a CI script. Detailed input-level diffs (which specific inputs changed) are not yet native in BlueBricks — this table shows version changes and affected environments, not per-input deltas.
...
In production, publishing runs automatically on merge to main via a `bricks updateci` + publish CI step.
